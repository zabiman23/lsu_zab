...
type SDRTunerData struct {
	...
	DcBias           bool
	Agc              bool
	VizData          driver.VisualizationData
}
...
func main() {
	flag.StringVar(&sensorID, "id", "b210-001", "Unique ID for this sensor")
	flag.StringVar(&driverName, "driver", "uhd", "SDR driver to use (e.g., uhd, uhd_holoscan)")
	flag.StringVar(&driverInitArgs, "driver-args", "type=b200", "Arguments string for driver Init (e.g., 'addr=192.168.1.2')")
	flag.Parse()
	...
	driverFunc, ok := availableDrivers[driverName]
	if !ok {
		log.Fatalf("Error: Driver '%s' not found.", driverName)
	}
	sdrDev = driverFunc()
	log.Printf("Initializing driver '%s' with args: '%s'", driverName, driverInitArgs)
	var initArgs []string
	if driverInitArgs != "" {
		initArgs = []string{driverInitArgs}
	}
...
func sdrLoop(vizData chan VisualizationData) {
	...
	for {
		...
		// Process samples and send to visualization channel
