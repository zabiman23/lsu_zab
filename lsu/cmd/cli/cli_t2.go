...
import (
	...
	"luf.co/lsu/pkg/driver/t2"
)

var (
	availableDrivers = map[string]func() driver.SDRDriver{
		"t2": func() driver.SDRDriver { return &t2.HTLT2Driver{} },
	}
	selectedDriver driver.SDRDriver
	driverName     = "t2" // Default driver
)
...
func main() {
	...
	for {
		...
		// Add debug log to trace cmdName before sending to commands channel
		//fmt.Printf("DBG - Parsed command: %s\n", cmdName)
...
func commandLoop(commands <-chan Command, done chan<- struct{}) error {
	...
	for cmd := range commands {
		...
		case "tuner_get_visualization_data":
			err := handleTunerGetVisualizationData(dev, cmd.Args)
			if err != nil {
				fmt.Printf("Get tuner visualization data failed: %v\n", err)
			}
...
func handleTunerGetVisualizationData(dev driver.SDRDriver, args []string) error {
	index, err := getTunerIndex(args)
	if err != nil {
		return err
	}
	...
	data := tuners[index].GetVisualizationData()
	fmt.Printf("Tuner %d Visualization Data: %v\n", index, data)
	return nil
}
...
func handleTunerStart(dev driver.SDRDriver, args []string) error {
	...
	// Parse --args flag if provided
	var ip, macAddress string
	var port uint32
	if len(args) == 5 && args[1] == "--args" {
		ip = args[2]
		...
		if err != nil || parsedPort <= 0 || parsedPort > 65535 {
			return fmt.Errorf("invalid port: %s", args[3])
		}
	...
	// Populate IP, Port, and MAC address fields in HTLT2RXTuner
	tuner := tuners[index].(*t2.HTLT2RXTuner)
	tuner.SIp = ip
	tuner.SPort = port
	tuner.SMacAddr = macAddress
...
func handleHelp() {
	...
	fmt.Println("  tuner_get_visualization_data [index] - Get visualization data from a tuner")
...
func handleInit(dev driver.SDRDriver, args []string) error {
	...
	ip := readInput("Enter IP: ", isValidIP)
	if ip == "" {
		return fmt.Errorf("failed to read valid IP")
	}
	port := readInput("Enter Port: ", isValidPort)
	if port == "" {
		return fmt.Errorf("failed to read valid Port")
	}

	args = []string{ip, port}

	// Simulate SDR device initialization
	err := dev.Init(args)
	if err != nil {
...
// Read user input asynchronously using a goroutine
func readInput(prompt string, validate func(string) bool) string {

	reader := bufio.NewReader(os.Stdin)
	for {
		fmt.Print(prompt) // Display the prompt only once per input attempt
		input, err := reader.ReadString('\n')
		fmt.Printf("Debug: Raw input received: [%q], Error: %v\n", input, err)
		if err != nil {
			fmt.Printf("Error reading input: %v\n", err)
			continue
		}

		// Trim all whitespace characters, including newlines and carriage returns
		input = strings.TrimSpace(input)
		fmt.Printf("Debug: Trimmed input: [%s]\n", input)
...
// isValidPort checks if the provided string is a valid port number (1-65535).
func isValidPort(port string) bool {
	p, err := strconv.Atoi(port)
	fmt.Printf("Debug: Port=%s, Parsed=%d, Error=%v\n", port, p, err)
