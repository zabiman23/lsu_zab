...
func main() {
	flag.StringVar(&sensorID, "id", "htl-t2", "Unique ID for this sensor")
	flag.StringVar(&driverName, "driver", "t2", "Name of the SDR driver to use")
	flag.StringVar(&driverInitArgs, "args", "", "Arguments string for driver initialization")
	flag.Parse()
	...
	sdrDev = t2.NewHTLT2Driver(sensorID)
	if err := sdrDev.Init(strings.Fields(driverInitArgs)); err != nil {
		log.Fatalf("Failed to initialize driver '%s': %v", driverName, err)
	}
