...
func main() {
	flag.StringVar(&sensorID, "id", "x40-001", "Unique ID for this sensor")
	flag.Parse()
	...
	sdrDev = x40.NewX40Driver(sensorID)
	initArgs := []string{"type=x40"}
...
func sdrLoop(vizData chan VisualizationData, done chan bool) {
	...
	defer func() {
		...
		log.Println("SDR loop signalling it's done...")
		done <- true
	}()
